...
#' @rdname plot_latent_mnorm
#' @export

plot_latent_mnorm <- function(smc,  habnams, nlv,wqlv = 1, xlim = c(-3, 3), xlab = '', ylab = ''){
	dimnam <- attr(smc$stat, 'dimnames')[[1]]
	ihab <- grepl('all.params',dimnam)
	inot <- grepl('1]', dimnam) | grepl(paste0(nlv+2,']'), dimnam)
	ihab <- which(ihab!=inot)
